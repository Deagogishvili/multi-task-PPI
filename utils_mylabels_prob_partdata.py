...
def read_filenames(data_list, part_ppi):
    ...
    filenames = filenames[::part_ppi]
    print("NOTE: ONLY TRAINED ON PART DATASET (1/{})".format(part_ppi))
    print("Number of sequences in dataset: {}".format(len(filenames)))
...
def read_labels(filenames, labels_files_path, fastas_files_path, data_enhance, indices):
    """
...
class InputReader(object):
    ...
    def __init__(self, data_list, inputs_files_path, labels_files_path, fastas_files_path,\
                 num_batch_size, part_ppi = 1, input_norm=False, shuffle=False, data_enhance=False):

        self.filenames = read_filenames(data_list, part_ppi)
    ...
    def read_file_from_disk(self, filenames_batch):
        ...
        labels_batch, labels_masks_batch, labels_total_len = \
            read_labels(filenames_batch, self.labels_files_path, self.fastas_files_path, self.data_enhance, indices)
