...
@author: xugang
@edit: Henriette Capel 19 Mar 2021
...
def main(tensorboard_dir, training_path, validation_path, test_path, output_erroranalyse, lr, class_imbalance_major, class_imbalance_minor, part_ppi_anno):
    ...
    model_IFBUSA = Model(params=params, name="IFBUSA")
    ...
    def train_step(x, x_mask, y, y_mask):
        ...
        ppi_predictions = buried_predictions = asa_predictions = None

        with tf.GradientTape() as tape:
             ppi_predictions, buried_predictions, asa_predictions, loss = model_IFBUSA.inference(x, x_mask, y, y_mask, training=True)

        trainable_variables = model_IFBUSA.transformer.trainable_variables + \
            model_IFBUSA.cnn.trainable_variables + model_IFBUSA.birnn.trainable_variables
        gradients = tape.gradient(loss, trainable_variables)
        optimizer.apply_gradients(
            zip(gradients, trainable_variables))

        return loss, ppi_predictions, buried_predictions, asa_predictions

    def infer_step(x, x_mask):

        ppi_predictions = asa_predictions = buried_predictions = None

        ppi_predictions, buried_predictions, asa_predictions, _ = model_IFBUSA.inference(x, x_mask, y, y_mask, training=False)
    ...
    for epoch in range(epochs):
        ...
        for step, filenames_batch in enumerate(train_reader.dataset):
            ...
            loss, ppi_predictions, buried_predictions, asa_predictions = train_step(x, x_mask, y, y_mask)
            ...
            if step % 20  == 0:
                ...
                if len(probability_list_train) > 0:
                    if sum(target_list_train) >= 1:
                        ...
                        print('Epoch: %d, step: %d, loss: %3.3f, acc_bur: %3.4f, pear_asa: %3.4f, acc_ppi: %3.4f, AUC_ROC: %3.4f, AUC_PR: %3.4f, prec: %3.4f, recall: %3.4f, TP: %0.1f, FP: %0.1f, TN: %0.1f, FN: %0.1f, time: %3.3f'
                            % (epoch, step, loss, np.mean(accuracy_train_bur), np.mean(pearson_train_asa), np.mean(accuracy_train_ppi), auc_roc, auc_pr, train_precision.result(), train_recall.result(), train_TP.result(), train_FP.result(), train_TN.result(), train_FN.result(), run_time))
                        if writer:
                            with writer.as_default():
                                ...
                                tf.summary.scalar("accuracy_buried", np.mean(accuracy_train_bur), step_train)
        ...
        for step, filenames_batch in enumerate(val_reader.dataset):
            ...
            ppi_predictions, buried_predictions, asa_predictions = infer_step(x, x_mask)
        ...
        print('Epoch: %d, lr: %s, acc_bur: %3.4f, pear_asa: %3.4f, acc_ppi: %3.4f, AUC_ROC: %3.4f, AUC_PR: %3.4f,, prec: %3.4f, recall: %3.4f, TP: %0.1f, FP: %0.1f, TN: %0.1f, FN: %0.1f, time: %3.3f'
            % (epoch, str(lr.numpy()), np.mean(accuracy_val_bur), np.mean(pearson_val_asa), np.mean(accuracy_val_ppi), auc_roc_val, auc_pr_val, val_precision.result(), val_recall.result(), val_TP.result(), val_FP.result(), val_TN.result(), val_FN.result(), run_time))
        ...
        if auc_roc_val > best_auc_roc:
            ...
            best_acc_bur = np.mean(accuracy_val_bur)
            best_pearson_asa= np.mean(pearson_val_asa)
        ...
        if writer:
            with writer.as_default():
                ...
                tf.summary.scalar("epoch_accuracy_bur", np.mean(accuracy_train_bur), epoch)
        ...
        if val_writer:
            with val_writer.as_default():
                ...
                tf.summary.scalar("epoch_accuracy_bur", np.mean(accuracy_val_bur), epoch)
    ...
    print('Epoch: %d, acc_bur: %3.4f, pear_asa: %3.4f, acc_ppi: %3.4f, AUC_ROC: %3.4f, AUC_PR: %3.4f, prec: %3.4f, recall: %3.4f, TP: %0.1f, FP: %0.1f, TN: %0.1f, FN: %0.1f'
          % (best_epoch, best_acc_bur, best_pearson_asa, best_acc, best_auc_roc, best_auc_pr, best_precision, best_recall, best_TP, best_FP, best_TN, best_FN))
    ...
    # model_IFBUSA_test = Model(params=params, name="IFBUSA")
    # model_IFBUSA_test.load_model()
    #
    ...
    #         model_IFBUSA_test.inference(x, x_mask, y, y_mask, training=False)
    #
    #     return ppi_predictions, buried_predictions, asa_predictions
    #
    # accuracy_test_ppi = []
