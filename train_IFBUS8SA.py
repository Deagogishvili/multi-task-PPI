...
def main(tensorboard_dir, training_path, validation_path, test_path, lr, class_imbalance_major, class_imbalance_minor):
    ...
    ############################################################################
    model_IFBUS8SA = Model(params=params, name="IFBUS8SA")
    ...
    def train_step(x, x_mask, y, y_mask):
        ...
        ppi_predictions = buried_predictions = ss8_predictions = asa_predictions = None

        with tf.GradientTape() as tape:
             ppi_predictions, buried_predictions, ss8_predictions, asa_predictions, loss = model_IFBUS8SA.inference(x, x_mask, y, y_mask, training=True)

        trainable_variables = model_IFBUS8SA.transformer.trainable_variables + \
            model_IFBUS8SA.cnn.trainable_variables + model_IFBUS8SA.birnn.trainable_variables
        gradients = tape.gradient(loss, trainable_variables)
        optimizer.apply_gradients(
            zip(gradients, trainable_variables))

        return loss, ppi_predictions, buried_predictions, ss8_predictions, asa_predictions

    def infer_step(x, x_mask):

        ppi_predictions = buried_predictions = ss8_predictions = asa_predictions = None

        ppi_predictions, buried_predictions, ss8_predictions, asa_predictions, _ = model_IFBUS8SA.inference(x, x_mask, y, y_mask, training=False)

        return ppi_predictions, buried_predictions, ss8_predictions, asa_predictions
    ...
    for epoch in range(epochs):
        ...
        for step, filenames_batch in enumerate(train_reader.dataset):
            ...
            loss, ppi_predictions, buried_predictions, ss8_predictions, asa_predictions = train_step(x, x_mask, y, y_mask)
            ...
            if step % 20  == 0:
                ...
                if len(target_list_train) > 0:
                    if sum(target_list_train) >= 1 and sum(target_list_train) < len(target_list_train):     #Needed because can be at begining of epoch and than all predictions can be IF or NIF
                        ...
                        print('Epoch: %d, step: %d, loss: %3.3f, pear_asa: %3.4f, acc_ss8: %3.4f, acc_bur: %3.4f, acc_ppi: %3.4f, AUC_roc: %3.4f, AUC_pr: %3.4f, prec: %3.4f, recall: %3.4f, TP: %0.1f, FP: %0.1f, TN: %0.1f, FN: %0.1f, time: %3.3f'
                            % (epoch, step, loss,  np.mean(pearson_train_asa), np.mean(accuracy_train_ss8), np.mean(accuracy_train_buried), np.mean(accuracy_train_ppi), auc_roc, auc_pr, train_precision.result(), train_recall.result(), train_TP.result(), train_FP.result(), train_TN.result(), train_FN.result(), run_time))
                        if writer:
        ...
        for step, filenames_batch in enumerate(val_reader.dataset):
            ...
            ppi_predictions, buried_predictions, ss8_predictions, asa_predictions = infer_step(x, x_mask)
        ...
        print('Epoch: %d, lr: %s, pear_asa: %3.4f, acc_ss8: %3.4f, acc_bur: %3.4f, acc_ppi: %3.4f, AUC_roc: %3.4f, AUC_pr: %3.4f, prec: %3.4f, recall: %3.4f, TP: %0.1f, FP: %0.1f, TN: %0.1f, FN: %0.1f, time: %3.3f'
            % (epoch, str(lr.numpy()), np.mean(pearson_val_asa), np.mean(accuracy_val_ss8), np.mean(accuracy_val_buried), np.mean(accuracy_val_ppi), auc_roc_val, auc_pr_val, val_precision.result(), val_recall.result(), val_TP.result(), val_FP.result(), val_TN.result(), val_FN.result(), run_time))
    ...
    print('Epoch: %d, pear_asa: %3.4f, acc_ss8: %3.4f, acc_bur: %3.4f, acc_ppi: %3.4f, AUC_roc: %3.4f, AUC_pr: %3.4f, prec: %3.4f, recall: %3.4f, TP: %0.1f, FP: %0.1f, TN: %0.1f, FN: %0.1f'
          % (best_epoch, best_pear_asa, best_acc_ss8, best_acc_bur, best_acc, best_auc_roc, best_auc_pr, best_precision, best_recall, best_TP, best_FP, best_TN, best_FN))
    ...
    # #======================Test======================
    #
    # model_IFBUS8SA_test = Model(params=params, name="IFBUS8SA")
    # model_IFBUS8SA_test.load_model()
    #
    # def test_infer_step(x, x_mask):
    #
    #     ppi_predictions = buried_predictions = ss8_predictions = asa_predictions = None
    #
    #     ppi_predictions, buried_predictions, ss8_predictions, asa_predictions, _ = \
    #         model_IFBUS8SA_test.inference(x, x_mask, y, y_mask, training=False)
    #
    #     return ppi_predictions, buried_predictions, ss8_predictions, asa_predictions
    #
    # accuracy_test_ppi = []
    ...
    #     assert inputs_total_len == labels_total_len
    #
    #     ppi_predictions, buried_predictions, ss8_predictions, asa_predictions = \
    ...
    #
    # print('pear_asa: %3.4f, acc_ss8: %3.4f, acc_bur: %3.4f, acc_ppi: %3.4f, AUC_roc: %3.4f, AUC_pr %3.4f, prec: %3.4f, recall: %3.4f, TP: %0.1f, FP: %0.1f, TN: %0.1f, FN: %0.1f, time: %3.3f'
    #     % (np.mean(pearson_test_asa), np.mean(accuracy_test_ss8), np.mean(accuracy_test_buried), np.mean(accuracy_test_ppi), auc_roc_test, auc_pr_test, test_precision.result(), test_recall.result(), test_TP.result(), test_FP.result(), test_TN.result(), test_FN.result(), run_time))
